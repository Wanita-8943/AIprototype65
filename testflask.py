...
##api (รับ input)
@app.route('/getrequest',methods=['GET']) #GET ส่งแบบตรงๆ
...
# ##api port
@app.route('/postrequest',methods=['POST']) #GET ส่งแบบตรงๆ
def port_request_detail():
    payload = request.data.decode("utf-8")
    inmessage = json.loads(payload) # payload คือของที่ส่งมาให้เรา
...
def home2():
    ...
    return render_template("home.html",name = 'Nita')#, fav = name)
    
if __name__ == "__main__":
    app.run(host='0.0.0.0',debug=True,port=5002)#host='0.0.0.0',port=5001